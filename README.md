...
- **数据存储**：所有附加数据都通过 **JSON 序列化** 的方式，将整个 `DrawerBox` 对象转换成一个字符串，并存储在实体的扩展数据（XData）中，应用程序名为 `YZ_FURNITURE_DATA`。
- **写入逻辑**：为确保逻辑的统一和代码的简洁，所有的数据写入操作都通过调用 `DrawerBox` 实例的 `WriteDataToEntity` 方法来完成。
- **扩展性**：此方案具有极佳的扩展性。未来增加新字段时，只需在 `DrawerBox.cs` 类中添加相应的属性即可，数据的读取和写入逻辑（基于JSON序列化）无需任何修改。
